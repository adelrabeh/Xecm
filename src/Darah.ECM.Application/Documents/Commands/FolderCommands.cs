...
public sealed class CreateFolderCommandValidator : AbstractValidator<CreateFolderCommand>
{
    public CreateFolderCommandValidator()
    {
        RuleFor(x => x.NameAr).NotEmpty().MaximumLength(300)
            .WithMessage("اسم المجلد مطلوب");
        RuleFor(x => x.LibraryId).GreaterThan(0)
            .WithMessage("يجب تحديد المكتبة");
    }
}
    ...
    : IRequestHandler<CreateFolderCommand, ApiResponse<FolderDto>>
{
    ...
    public CreateFolderCommandHandler(IFolderRepository folderRepo,
        ICurrentUser user, IAuditService audit)
        { _folderRepo = folderRepo; _user = user; _audit = audit; }

    public async Task<ApiResponse<FolderDto>> Handle(
        CreateFolderCommand cmd, CancellationToken ct)
    {
        ...
        if (cmd.ParentFolderId.HasValue)
        {
            ...
            if (parent is null)
                return ApiResponse<FolderDto>.Fail("المجلد الأب غير موجود");
            parentPath = parent.Path;
            depth      = parent.DepthLevel + 1;
        }

        // Max depth guard (prevent runaway nesting)
        if (depth > 10)
            return ApiResponse<FolderDto>.Fail("لا يمكن إنشاء مجلدات بعمق أكثر من 10 مستويات");
        ...
        // Set materialized path now that FolderId is known
        folder.SetPath(parentPath, depth);
        await _folderRepo.CommitAsync(ct);

        await _audit.LogAsync("FolderCreated", "Folder", folder.FolderId.ToString(),
            newValues: new { folder.NameAr, folder.LibraryId, folder.ParentFolderId }, ct: ct);

        return ApiResponse<FolderDto>.Ok(MapToDto(folder), "تم إنشاء المجلد بنجاح");
    }

    private static FolderDto MapToDto(Folder f) => new(
        f.FolderId, f.NameAr, f.NameEn, f.LibraryId, f.ParentFolderId,
        f.Path, f.DepthLevel, f.SortOrder, f.CreatedAt);
}
...
public sealed class MoveFolderCommandHandler
    : IRequestHandler<MoveFolderCommand, ApiResponse<bool>>
{
    private readonly IFolderRepository _folderRepo;
    private readonly ICurrentUser      _user;
    private readonly IAuditService     _audit;

    public MoveFolderCommandHandler(IFolderRepository folderRepo,
        ICurrentUser user, IAuditService audit)
        { _folderRepo = folderRepo; _user = user; _audit = audit; }
    ...
    public async Task<ApiResponse<bool>> Handle(MoveFolderCommand cmd, CancellationToken ct)
    {
        ...
        if (cmd.NewParentFolderId.HasValue)
        {
            ...
            folder.Move(cmd.NewParentFolderId, $"{newParent.Path}{cmd.FolderId}/",
                newParent.DepthLevel + 1, _user.UserId);
        }
        else
        {
...
// Document DTOs referenced from DocumentQueries
